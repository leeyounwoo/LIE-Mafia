...
public class GameTurnImpl implements GameTurn{
    ...
    public void setnextWork(String roomId, Timer timer) {
        ...
        save_time.put(RoomPhase.NIGHTVOTE,90);
        save_time.put(RoomPhase.MORNING,120);
        save_time.put(RoomPhase.MORNINGVOTE,90);
        save_time.put(RoomPhase.FINALSPEECH,30);
        save_time.put(RoomPhase.EXECUTIONVOTE,60);
        ...
        TimerTask timerTask2 = new TimerTask(){
            ...
            public void run() {
                room = roomRepository.findById(roomId).orElseThrow();
                //다음이 최후 변론인데 Result가 null인 경우
                if(room.getRoomPhase().equals(RoomPhase.FINALSPEECH) && room.getResult() == null){
                    room.setRoomPhase(RoomPhase.NIGHTVOTE);
                }
